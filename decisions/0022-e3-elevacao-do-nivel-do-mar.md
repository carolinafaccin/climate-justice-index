...
1. Calcular distância euclidiana de cada pixel terrestre ao oceano via `fastDistanceTransform`.
1. Marcar pixel **em risco** se elevação ≤ 1 m **AND** distância ao oceano ≤ 10 km — limiar que inclui estuários e planícies costeiras legítimas, excluindo baixadas fluviais interiores.
1. Fração de área em risco por hexágono via `Reducer.mean()` com buffer de 174 m (circunraio H3 res9).
1. Indicador final: produto `qtd_dom × risco_slr` (quantidade de domicílios multiplicada pela fração de risco).
