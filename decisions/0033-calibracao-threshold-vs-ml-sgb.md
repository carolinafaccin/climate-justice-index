...
1. **Circularidade com os indicadores existentes**: LHASA e HAND já são proxies dos fatores físicos que um modelo de ML usaria (declividade, geologia, distância à drenagem, cobertura vegetal). Treinar ML a partir de LHASA/HAND é circular — aprende o que já sabemos. Treinar a partir de variáveis brutas (DTM, geologia, solo) exigiria curadoria de bases nacionais em escala compatível, o que está fora do escopo do IIC.

1. **Cobertura nacional já existe**: o propósito dos dados SGB no IIC não é criar mapas de suscetibilidade para municípios não cobertos — é calibrar os thresholds dos indicadores nacionais já disponíveis. Após calibração, LHASA e HAND cobrem os ~5.570 municípios.

1. **Interpretabilidade e auditabilidade**: o threshold resultante ("lhasa_high_frac > 0.X") é comunicável e auditável em uma linha. Um modelo de ML não é.
