...
1. Expandir para **buffer ~600 m** via `grid_disk(k=2)` — vizinhança hexagonal de raio 2.
1. Computar **fração de anos** (2016–2025) em que o hexágono ou sua vizinhança teve ao menos um foco.
1. **Critério de exposição recorrente**: hexágonos com foco em apenas 1 ano dos 10 recebem zero (eventos isolados não caracterizam risco crônico). Apenas fração ≥ 0.2 (≥ 2 anos) entra no cálculo.
