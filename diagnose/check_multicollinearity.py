"""
Diagnóstico de multicolinearidade entre indicadores normalizados do IIC.

Gera:
  1. Heatmap de correlação de Spearman salvo em outputs/figures/
  2. Lista de pares com |r| >= THRESHOLD no terminal

Usa as colunas _norm (valores que entram no índice, após inversão),
com amostragem aleatória para viabilizar o cálculo em ~4.5M hexágonos.
"""
...
THRESHOLD  = 0.70     # |r| acima disso é sinalizado como multicolinearidade
MAX_SAMPLE = 300_000  # hexágonos amostrados para velocidade

# ==============================================================================
# 1. CARREGAR INDICADORES NORMALIZADOS
# ==============================================================================
print("Carregando indicadores normalizados...")
...
for key, path in cfg.FILES_H3.items():
    if key == "base_metadata":
        continue
    if not path.exists():
        print(f"  ✗ {key:<4} — arquivo não encontrado")
        continue
    ...
    try:
        ...
        print(f"  ✗ {key:<4} — ERRO: {e}")

# Ordenar por dimensão conforme indicators.json
ordered_keys = [
    key
...
print(f"\nTotal de hexágonos: {len(df_all):,}  |  Indicadores: {len(ordered_keys)}")

# Amostragem
if len(df_all) > MAX_SAMPLE:
    df_sample = df_all.sample(MAX_SAMPLE, random_state=42)
    sample_note = f"amostra {MAX_SAMPLE:,} hexágonos"
else:
    df_sample = df_all
    sample_note = f"{len(df_all):,} hexágonos"
...
# ==============================================================================
# 3. TERMINAL — PARES COM |r| >= THRESHOLD
# ==============================================================================
key_meta = {}
...
print(f"\n{'─' * 72}")
print(f"Pares com |r| ≥ {THRESHOLD}  ({len(pairs)} encontrado{'s' if len(pairs) != 1 else ''}):")
print(f"{'─' * 72}")
if pairs:
...
# Paleta de cores por dimensão (uma cor por dim, gerada dinamicamente)
...
# Labels de eixo: "p1 / mul"
labels = [f"{k} / {key_meta[k]['abbr']}" for k in ordered_keys]

# Blocos de dimensão (posição inicial + tamanho)
dim_blocks = []
pos = 0
for dim in dim_list:
...
# Borda dourada nas células com |r| >= THRESHOLD (exceto diagonal)
for i, k1 in enumerate(ordered_keys):
...
# Colorir tick labels pela dimensão do indicador
...
ax.set_title(
    f"Multicolinearidade entre indicadores normalizados — IIC {cfg.INDEX_VERSION}\n"
    f"Correlação de Spearman  |  {sample_note}  |  destaque dourado: |r| ≥ {THRESHOLD}",
    fontsize=12, pad=16,
)
