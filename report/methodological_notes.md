...
Exposição crônica a focos de queimadas em hexágonos habitados, medida pela fração de anos (2016–2025) com ao menos um foco de calor na vizinhança (~1 km) do hexágono. Hexágonos desabitados recebem pontuação zero.

Para cada ano, as coordenadas dos focos foram convertidas em hexágonos H3 (resolução 9) e expandidas para um raio de aproximadamente 1 km via operação `grid_disk(k=4)`. A fração de anos com ao menos um foco na vizinhança do hexágono (0 a 1) é o indicador bruto. Normalização min-max com winsorização (p1%–p99%), sem ponderação pela quantidade de domicílios — seguindo o mesmo princípio do indicador E4, a escala populacional é capturada nas dimensões de grupos prioritários e vulnerabilidade.
